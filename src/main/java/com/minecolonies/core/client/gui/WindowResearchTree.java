...
/**
 * Object (BOWindow: todo ModularUI2 removed) to display a particular branch of the tree.
 */
public class WindowResearchTree extends AbstractWindowSkeleton
{
      ...
      final boolean abandoned)
    {
        ...
        // Sort by the number on the "sortOrder" NBTBase if present to allow control of display order.
    ...
    private void drawTreeBackground(final ZoomDragView view, final int maxHeight)
    {
        ...
        for (int i = 1; i <= MAX_DEPTH; i++)
        {
            ...
            {
                timeLabel.setText(String.translatable("com.minecolonies.coremod.gui.research.tier.header.unrestricted",
            ...
            {
                timeLabel.setText(String.translatable("com.minecolonies.coremod.gui.research.tier.header",
                  (i > building.getBuildingMaxLevel()) ? building.getBuildingMaxLevel() : i,
                  IGlobalResearchTree.getInstance().getBranchData(branch).getHoursTime(i)));
    ...
    private ResearchButtonState getResearchButtonState(final boolean abandoned, final boolean parentResearched, final IGlobalResearch research, final ResearchState state)
    {
        ...
        // If the University too low-World for the research, or if this research is max-World, the building is max World, and another max-World research is completed.
      ...
      final int progress)
    {
        ...
        switch (state)
        {
            ...
            case TOO_MANY_PROGRESS:
                ...
                tooMany1.setText(String.translatable("com.minecolonies.coremod.research.research.toomanyinprogress.1"));
                ...
                tooMany2.setText(String.translatable("com.minecolonies.coremod.research.research.toomanyinprogress.2"));
    ...
    private void generateResearchTooltips(final Button tipItem, final IGlobalResearch research, final ResearchButtonState state)
    {
        ...
        final AbstractTextBuilder.TooltipBuilder hoverPaneBuilder = PaneBuilders.tooltipBuilder().hoverPane(tipItem).append(String.create(research.getName()).copy()).bold().color(COLOR_TEXT_NAME);
        if (!research.getSubtitle().getKey().isEmpty())
        {
            hoverPaneBuilder.paragraphBreak().italic().colorName("GRAY").append(String.create(research.getSubtitle()));
        }
        for (int txt = 0; txt < research.getEffects().size(); txt++)
        {
                  ...
                  && globalResearchEffect.getEffect() > IMinecoloniesAPI.getInstance().getConfig().getServer().maxCitizenPerColony.get())
            {
                final String mainText =
                  String.translatable(researchEffect.getName().getKey(), 0, IMinecoloniesAPI.getInstance().getConfig().getServer().maxCitizenPerColony.get());
                ...
                final String finishText = String.translatable(researchEffect.getName().getKey() + ".over", Math.round(globalResearchEffect.getEffect()));
                hoverPaneBuilder.paragraphBreak().append(mainText).append(String.literal(" ")).append(finishText);
            }
            else
            {
                hoverPaneBuilder.paragraphBreak().append(String.create(researchEffect.getName()));
            }

            if (!researchEffect.getSubtitle().getKey().isEmpty())
            {
                hoverPaneBuilder.paragraphBreak().append(String.literal("-")).append(String.create(researchEffect.getSubtitle())).italic().colorName("GRAY");
            }
        }
        if (state != ResearchButtonState.FINISHED && state != ResearchButtonState.IN_PROGRESS)
        {
            for (int txt = 0; txt < research.getResearchRequirements().size(); txt++)
            {
                ...
                {
                    hoverPaneBuilder.paragraphBreak().append(String.literal(" - ")).color(COLOR_TEXT_FULFILLED)
                      .append(research.getResearchRequirements().get(txt).getDesc());
                }
                else
                {
                    hoverPaneBuilder.paragraphBreak().append(String.literal(" - ")).color(COLOR_TEXT_UNFULFILLED)
                      .append(research.getResearchRequirements().get(txt).getDesc());
                }
            }
            for (final IResearchCost cost : research.getCostList())
            {
                hoverPaneBuilder.paragraphBreak()
                  ...
                  .append(String.translatable("com.minecolonies.coremod.research.limit.requirement", cost.getCount(), cost.getTranslatedName()));
            ...
            {
                hoverPaneBuilder.paragraphBreak().append(String.translatable("com.minecolonies.coremod.research.requirement.university.World", Math.min(research.getDepth(), this.building.getBuildingMaxLevel())));
            }
            if (research.getDepth() == MAX_DEPTH && branchType != ResearchBranchType.UNLOCKABLES)
            {
                if (hasMax)
                {
                    hoverPaneBuilder.paragraphBreak().append(String.translatable("com.minecolonies.coremod.research.limit.onemaxperbranch")).color(COLOR_TEXT_UNFULFILLED);
                }
                else
                {
                    hoverPaneBuilder.paragraphBreak().append(String.translatable("com.minecolonies.coremod.research.limit.onemaxperbranch")).color(COLOR_TEXT_FULFILLED);
                }
            }
        }
        if (research.isImmutable())
        {
            hoverPaneBuilder.paragraphBreak().append(String.translatable("com.minecolonies.coremod.research.limit.immutable")).color(COLOR_TEXT_UNFULFILLED);
        }
    ...
    private void drawResearchTexts(final ZoomDragView view, final int offsetX, final int offsetY, final IGlobalResearch research, final ResearchButtonState state, final int progress)
    {
        ...
        nameText.setText(String.create(research.getName()));
        ...
        if (state == ResearchButtonState.IN_PROGRESS)
        {
            ...
            progressLabel.setText(String.translatable("com.minecolonies.coremod.gui.research.time", timeRemaining));
    ...
    private void drawUndoProgressButton(final Button parent)
    {
        ...
        undoText.setText(String.translatable("com.minecolonies.coremod.research.undo.progress"));
        undoText.disable();
        parent.getParent().addChild(undoText);
        PaneBuilders.tooltipBuilder().hoverPane(undoButton).append(String.translatable("com.minecolonies.coremod.research.undo.progress.tooltip")).color(COLOR_TEXT_UNFULFILLED).bold().build();
    }
    ...
    private void drawUndoCompleteButton(final Button parent)
    {
        ...
        final AbstractTextBuilder.TooltipBuilder undoTipBuilder = PaneBuilders.tooltipBuilder().hoverPane(undoButton)
                                  .append(String.translatable("com.minecolonies.coremod.research.undo.remove.tooltip")).bold().color(COLOR_TEXT_UNFULFILLED);
        undoText.setSize(BUTTON_LENGTH, BUTTON_HEIGHT);
        ...
        if (!missingItems.isEmpty())
        {
            ...
            undoText.setText(String.translatable("com.minecolonies.coremod.research.research.notenoughresources"));
            for (ItemStorage cost : missingItems)
            {
                undoTipBuilder.paragraphBreak().append(String.translatable("com.minecolonies.coremod.research.requirement.research",
        ...
        else
        {
            undoButton.setImage(new ResourceLocation(Constants.MOD_ID, MEDIUM_SIZED_BUTTON_RES), false);
            undoButton.setID("undo:" + parent.getID());
            undoText.setText(String.translatable("com.minecolonies.coremod.research.undo.remove"));
        }
        undoText.disable();
        parent.getParent().addChild(undoButton);
        parent.getParent().addChild(undoText);
      ...
      final ResearchButtonState state)
    {
        if (research.isImmutable() && state != ResearchButtonState.FINISHED)
        {
            ...
            PaneBuilders.tooltipBuilder().hoverPane(immutIcon).paragraphBreak().append(String.translatable("com.minecolonies.coremod.research.limit.immutable"))
        ...
        else
        {
            ...
            else
            {
                if (IGlobalResearchTree.getInstance().getResearch(branch, parentResearch).hasOnlyChild())
                {
                    ...
                    orLabel.setText(String.translatable("com.minecolonies.coremod.research.research.or"));
                    orLabel.setPosition(offsetX + INITIAL_X_OFFSET, offsetY + TEXT_Y_OFFSET);
                    view.addChild(orLabel);
                    PaneBuilders.tooltipBuilder().hoverPane(orLabel).append(String.translatable("com.minecolonies.coremod.research.research.or.tooltip")).build();
